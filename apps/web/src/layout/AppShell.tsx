...
const productGroups: ProductGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/holo/outputs', label: 'Thành phẩm', icon: Package },
      { path: '/holo/history', label: 'Lịch sử', icon: History },
      { path: '/holo/settings', label: 'Cấu hình', icon: Settings },
    ],
  },
  { title: 'Label', label: 'Label', icon: Tag, defaultOpen: false, items: [
    { path: '/label', label: 'Tổng quan', icon: LayoutDashboard }, { path: '/label/queue', label: 'PDF chờ', icon: ListTodo }, { path: '/label/download', label: 'Tải PDF từ Excel', icon: Download }, { path: '/label/tool', label: 'Chạy Tool', icon: Layers3 }, { path: '/label/done', label: 'Đã xong', icon: CheckCircle2 }, { path: '/label/errors', label: 'Lỗi', icon: AlertTriangle }, { path: '/label/outputs', label: 'Thành phẩm', icon: Package }, { path: '/label/settings', label: 'Cấu hình', icon: Settings },
  ] },
...
function SidebarItem({ item }: { item: NavItem }) {
  ...
  return <NavLink to={item.path} reloadDocument end title={item.label} className={({ isActive }) => cn('flex items-center gap-3 rounded-2xl px-3 py-2.5 text-[15px] font-medium', isActive ? 'bg-blue-600 text-white' : 'text-slate-700 hover:bg-slate-100')}><Icon className="h-4 w-4 shrink-0"/><span className="whitespace-nowrap opacity-0 group-hover/sidebar:opacity-100">{item.label}</span></NavLink>;
}
function ProductSection({ group, open, onToggle }: { group: ProductGroup; open: boolean; onToggle: () => void }) {
  const Icon = group.icon;
  return <section className="space-y-2"><button type="button" onClick={onToggle} title={group.label} className={cn('flex w-full items-center gap-3 rounded-2xl border px-3 py-3 text-left ', group.disabled ? 'border-slate-100 bg-slate-50 text-slate-400' : 'border-blue-100 bg-blue-50 text-blue-800 hover:bg-blue-100')}><Icon className="h-5 w-5 shrink-0"/><span className="whitespace-nowrap font-semibold opacity-0 group-hover/sidebar:opacity-100">{group.label}</span><ChevronDown className={cn('ml-auto h-4 w-4 opacity-0 group-hover/sidebar:opacity-100', open ? 'rotate-180' : 'rotate-0')} /></button><div className={cn('overflow-hidden pl-4 ', open ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0')}><div className="space-y-1 pt-1">{group.items.map((item) => <SidebarItem key={item.path} item={item} />)}</div></div></section>;
}
