...
function CheckSettingsPanel() {
  const isSticker = window.location.pathname === '/sticker' || window.location.pathname.startsWith('/sticker/');
  ...
  const [stickerSettings, setStickerSettings] = useState({ marginMm: 3, gapMm: 5 });
  ...
  useEffect(() => { if (isSticker) void fetch(apiBase + '/settings/sticker', { cache: 'no-store' }).then((response) => response.json()).then((payload: { stickerSettings?: { marginMm: number; gapMm: number } }) => { if (payload.stickerSettings) setStickerSettings(payload.stickerSettings); }).catch(() => {}); }, [isSticker]);
  ...
  const saveSticker = async () => { setSaving(true); setNotice(''); try { const response = await fetch(apiBase + '/settings/sticker', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(stickerSettings) }); const payload = await response.json() as { ok?: boolean; message?: string }; setNotice(payload.ok ? 'Đã lưu cấu hình Sticker. Lần chạy Sticker tiếp theo sẽ áp dụng.' : (payload.message ?? 'Không thể lưu cấu hình Sticker.')); } catch { setNotice('Không thể kết nối đến app local để lưu cấu hình Sticker.'); } finally { setSaving(false); } };
  if (isSticker) return <Panel title="Cấu hình Sticker Vinyl"><div className="grid gap-4 md:grid-cols-2"><label className="text-sm"><div className="mb-2 font-semibold text-slate-700">Khoảng cách với template (mm)</div><input type="number" min="0" step="0.1" value={stickerSettings.marginMm} onChange={(event) => setStickerSettings((current) => ({ ...current, marginMm: Math.max(0, Number(event.target.value || 0)) }))} className="h-11 w-full rounded-xl border border-slate-300 px-3" /><div className="mt-1 text-xs text-slate-500">Khoảng cách từ sticker đến mép vùng xếp của template. Mặc định 3mm.</div></label><label className="text-sm"><div className="mb-2 font-semibold text-slate-700">Khoảng cách giữa các sticker (mm)</div><input type="number" min="0" step="0.1" value={stickerSettings.gapMm} onChange={(event) => setStickerSettings((current) => ({ ...current, gapMm: Math.max(0, Number(event.target.value || 0)) }))} className="h-11 w-full rounded-xl border border-slate-300 px-3" /><div className="mt-1 text-xs text-slate-500">Khoảng hở giữa các sticker khi xếp. Mặc định 5mm.</div></label></div><div className="mt-4 flex items-center gap-3"><button type="button" disabled={saving} onClick={() => void saveSticker()} className="h-11 rounded-xl bg-blue-600 px-5 text-sm font-semibold text-white disabled:opacity-50">{saving ? 'Đang lưu...' : 'Lưu cấu hình Sticker'}</button>{notice ? <div className="text-sm text-slate-600">{notice}</div> : null}</div></Panel>;
...
function FolderPathEditor({ folders }: { folders: DashboardData['settings']['folders'] }) {
  ...
  const saveFolder = async (moveData: boolean) => {
    ...
    try { const response = await fetch(apiBase + '/settings/folders/save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ folderPaths: { [editing.key]: nextPath }, moveData }) }); const payload = await response.json() as { ok?: boolean; message?: string; moveData?: boolean; normalizedPaths?: Record<string, { inputPath: string; normalizedPath: string; mappedDrive?: string; uncResolved: boolean }> }; if (!payload.ok) { setNotice(payload.message ?? 'Không thể lưu đường dẫn.'); return; } setNotice(payload.moveData === false ? 'Đã lưu đường dẫn mới, không chuyển dữ liệu cũ.' : 'Đã lưu đường dẫn và chuyển dữ liệu cũ sang thư mục mới.'); window.setTimeout(() => { setEditing(null); window.dispatchEvent(new Event('acrylic:folders-changed')); }, 700); }
...
export function SettingsPage({ data }: { data: DashboardData }) {
  ...
  const folderLabels: Record<string, string> = {
    ...
    wait_meta: 'Wait Meta - dữ liệu vùng đã xếp',
  ...
  const folders = data.settings.folders.filter((folder) => !isSticker || stickerFolderKeys.has(folder.key)).map((folder) => ({ ...folder, label: folderLabels[folder.key] ?? folder.label }));
