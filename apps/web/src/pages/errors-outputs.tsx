...
export function ErrorsPage({ data }: { data: DashboardData }) {
  ...
  const [approveItem, setApproveItem] = useState<ErrorItem | null>(null);
  ...
  const confirmApprove = async () => {
    ...
    try {
      const response = await fetch(`${apiBase}/errors/approved`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ relativePath: approveItem.previewRelativePath ?? approveItem.fileName }) });
      const payload = await response.json() as { ok?: boolean; message?: string };
      if (!response.ok || !payload.ok) throw new Error(payload.message || 'Không thể approve ảnh.');
      setProcessedIds((current) => new Set(current).add(approveItem.id)); setNotice(payload.message ?? 'Đã approve ảnh.'); setApproveItem(null); window.dispatchEvent(new CustomEvent('acrylic:folders-changed'));
    } catch (error) { setNotice(error instanceof Error ? error.message : 'Không thể approve ảnh.'); } finally { setApproving(false); }
  };
  ...
  return <><div className="grid gap-6 xl:grid-cols-[1.45fr_0.95fr]"><div className="space-y-6"><SectionTitle title="Ảnh lỗi" subtitle="Tập hợp mọi lỗi để xử lý nhanh và rõ ràng."/>{notice ? <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">{notice}</div> : null}<div className="grid gap-4 xl:grid-cols-4"><MetricCard icon={TriangleAlert} label="Tổng lỗi" value={errorsWithImage.length} tone="red"/><MetricCard icon={AlertCircle} label="Sai kích thước" value={sizeErrors} tone="blue"/><MetricCard icon={Package} label="Front / Back" value={frontBackErrors} tone="violet"/><MetricCard icon={Crosshair} label="Lazer lệch" value={lazerErrors} tone="orange"/></div><Panel><FilterBar><FilterSelect label="Tất cả lỗi"/><FilterSelect label="Bước lỗi"/><FilterInput placeholder="Tìm tên file"/></FilterBar></Panel>{errorsWithImage.length > 0 ? <DataTable headers={['Ảnh', 'Tên file', 'Size', 'Side', 'Qty', 'Bước lỗi', 'Nguyên nhân', 'Thời gian', 'Thao tác']}>{paged.items.map((item) => <ErrorRow key={item.id} item={item} onProcessed={setProcessedItem} onApprove={setApproveItem}/>)}</DataTable> : <Panel><div className="py-10 text-center text-sm font-medium text-slate-500">Hiện không có ảnh lỗi thật trong folder images_error.</div></Panel>}<PaginationBar page={paged.currentPage} totalPages={paged.totalPages} totalItems={paged.total} pageSize={pageSize} onPageChange={setPage} onPageSizeChange={(size) => { setPageSize(size); setPage(1); }} /></div><DetailPanel title="Chi tiết lỗi">{selected ? <div className="space-y-4"><div className="flex justify-between"><div className="text-[18px] font-semibold">{selected.fileName}</div><StatusBadge label="FAIL" tone="red"/></div><div className="grid grid-cols-3 gap-2 rounded-2xl border border-slate-200 p-3 text-center"><div><div className="text-xs text-slate-500">Thực tế</div><div className="mt-2 text-xl font-semibold text-rose-600">{selected.actual}</div></div><div><div className="text-xs text-slate-500">Yêu cầu</div><div className="mt-2 text-xl font-semibold">{selected.expected}</div></div><div><div className="text-xs text-slate-500">Sai lệch</div><div className="mt-2 text-xl font-semibold text-rose-600">{selected.delta}</div></div></div><div className="grid grid-cols-3 gap-2"><FileThumbnail scope={selected.previewScope} relativePath={selected.previewRelativePath} fileName={selected.fileName} className="h-44 w-full"/><FileThumbnail scope={selected.previewScope} relativePath={selected.previewRelativePath} fileName={selected.fileName} className="h-44 w-full"/><div className="flex h-44 items-center justify-center rounded-2xl border border-slate-200 text-slate-300"><Crosshair className="h-24 w-24"/></div></div><div className="rounded-2xl border border-slate-200 p-4 text-sm"><div className="flex justify-between"><span>Sheet</span><b>{selected.sheet}</b></div><div className="mt-3 flex justify-between"><span>Lần chạy</span><b>{selected.runId}</b></div><div className="mt-3 text-rose-600">Bước lỗi: {selected.step} · {selected.reason}</div></div><div className="grid grid-cols-3 gap-3"><button type="button" onClick={() => setProcessedItem(selected)} className="inline-flex h-14 items-center justify-center rounded-2xl border border-emerald-300 bg-white px-6 text-[16px] font-medium text-emerald-700 hover:bg-emerald-50">Processed</button><ActionButton label="Về Images"/></div></div> : null}</DetailPanel></div>{processedItem ? <ProcessedModal fileName={processedItem.fileName} processing={processing} onClose={() => !processing && setProcessedItem(null)} onConfirm={() => void confirmProcessed()} /> : null}{approveItem ? <ApproveModal fileName={approveItem.fileName} approving={approving} onClose={() => !approving && setApproveItem(null)} onConfirm={() => void confirmApprove()} /> : null}</>;
}
...
export function OutputsPage({ data }: { data: DashboardData }) {
  const isSimpleOutput = /^\/(label|sticker|sticker-holo)(?:\/|$)/.test(window.location.pathname);
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const [search, setSearch] = useState('');
  const [flowFilter, setFlowFilter] = useState<'all' | 'fba' | 'fbm' | 'other'>('all');
  ...
  const renameSelected = async () => {
    const selectedGroups = filtered.filter((group) => selectedIds.has(group.id));
    if (!selectedGroups.length || !renameBase.trim()) return;
    try {
      const response = await fetch(`${apiBase}/outputs/rename`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ files: selectedGroups.map((group) => group.outputAiRelativePath ?? group.fileName), baseName: renameBase.trim() }) });
      const payload = await response.json() as { ok?: boolean; message?: string };
      setExportMessage(payload.message ?? 'Không thể đổi tên file.');
      if (payload.ok) { setSelectedIds(new Set()); setRenameBase(''); window.dispatchEvent(new CustomEvent('acrylic:folders-changed')); }
    } catch (error) { setExportMessage(error instanceof Error ? error.message : 'Không thể đổi tên file.'); }
  };
  const outputFlow = (fileName: string) => /^fba_/i.test(fileName) ? 'fba' : /^fbm_/i.test(fileName) ? 'fbm' : 'other';
  const flowCounts = useMemo(() => ({
    fba: data.outputs.filter((group) => outputFlow(group.fileName) === 'fba').length,
    fbm: data.outputs.filter((group) => outputFlow(group.fileName) === 'fbm').length,
    other: data.outputs.filter((group) => outputFlow(group.fileName) === 'other').length,
  }), [data.outputs]);
  const filtered = useMemo(() => data.outputs.filter((group) => {
    const matchesFlow = flowFilter === 'all' || outputFlow(group.fileName) === flowFilter;
    return matchesFlow && group.fileName.toLowerCase().includes(search.trim().toLowerCase());
  }), [data.outputs, flowFilter, search]);
  ...
  return <div className="space-y-6"><SectionTitle title="Thành phẩm" subtitle={isSimpleOutput ? "Các file AI thành phẩm đã được lưu." : "Mỗi dòng là một bộ output gồm AI, BACK, FRONT và LAZER."}/>{exportMessage ? <div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">{exportMessage}</div> : null}{toolStatus.running ? <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">{toolStatus.run?.kind === 'export' ? 'Đang export. Không thể chạy thêm export hoặc Tool cho đến khi hoàn tất.' : 'Tool đang chạy. Không thể export để tránh xung đột với Illustrator.'}</div> : null}<div className="grid gap-4 xl:grid-cols-4"><MetricCard icon={FileImage} label={flowFilter === 'all' ? 'File AI' : `File AI ${flowFilter.toUpperCase()}`} value={filtered.length} tone="orange"/>{!isSimpleOutput ? <><MetricCard icon={FileImage} label="BACK" value={data.summary.kpi.outputBack} tone="green"/><MetricCard icon={FileImage} label="FRONT" value={data.summary.kpi.outputFront} tone="blue"/><MetricCard icon={Crosshair} label="LAZER" value={data.summary.kpi.outputLazer} tone="red"/></> : null}</div><Panel>{isSimpleOutput ? <div className="mb-4 flex w-full flex-col gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-4"><div className="text-sm font-semibold text-blue-900">Chọn file rồi đổi tên</div><div className="flex gap-3"><input value={renameBase} onChange={(e) => setRenameBase(e.target.value)} placeholder="Tên mới, ví dụ FBA" className="h-12 flex-1 rounded-xl border border-slate-300 bg-white px-3"/><button type="button" disabled={!selectedIds.size || !renameBase.trim()} onClick={() => void renameSelected()} className="h-12 min-w-[150px] rounded-xl bg-blue-600 px-5 font-semibold text-white disabled:opacity-40">Đổi tên</button></div></div> : null}<div className="mb-4 flex flex-wrap gap-2"><button type="button" onClick={() => { setFlowFilter('all'); setPage(1); }} className={`h-10 rounded-xl px-4 text-sm font-semibold ${flowFilter === 'all' ? 'bg-slate-900 text-white' : 'border border-slate-300 bg-white text-slate-700'}`}>Tất cả ({data.outputs.length})</button><button type="button" onClick={() => { setFlowFilter('fba'); setPage(1); }} className={`h-10 rounded-xl px-4 text-sm font-semibold ${flowFilter === 'fba' ? 'bg-blue-600 text-white' : 'border border-blue-200 bg-blue-50 text-blue-800'}`}>FBA ({flowCounts.fba})</button><button type="button" onClick={() => { setFlowFilter('fbm'); setPage(1); }} className={`h-10 rounded-xl px-4 text-sm font-semibold ${flowFilter === 'fbm' ? 'bg-violet-600 text-white' : 'border border-violet-200 bg-violet-50 text-violet-800'}`}>FBM ({flowCounts.fbm})</button>{flowCounts.other ? <button type="button" onClick={() => { setFlowFilter('other'); setPage(1); }} className={`h-10 rounded-xl px-4 text-sm font-semibold ${flowFilter === 'other' ? 'bg-amber-600 text-white' : 'border border-amber-200 bg-amber-50 text-amber-800'}`}>Khác ({flowCounts.other})</button> : null}</div><div className="mb-4"><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Tìm theo tên file AI..." className="h-12 w-full rounded-2xl border border-slate-200 px-4 outline-none focus:border-blue-500 xl:max-w-md" /></div><DataTable headers={isSimpleOutput ? ['Ngày / Sheet', 'AI'] : ['Ngày / Sheet', 'AI', 'BACK', 'FRONT', 'LAZER', 'Export']}>{paged.items.map((group) => { const frontAsset = asset(group, 'FRONT'); const backAsset = asset(group, 'BACK'); const lazerAsset = asset(group, 'LAZER'); const missingAny = [frontAsset, backAsset, lazerAsset].some((item) => item.status !== 'exported'); const isCurrentExportGroup = toolStatus.running && toolStatus.run?.kind === 'export' && (toolStatus.run.outputAiRelativePath === (group.outputAiRelativePath ?? group.fileName)); return <tr key={group.id} className="align-top bg-white"><td className="px-4 py-4"><div className="font-semibold">{group.date}</div><div className="mt-1 text-sm text-slate-500">Sheet {group.sheet}</div><div className="mt-1 text-xs text-slate-400">{group.completedAt}</div></td><td className="px-3 py-4"><input type="checkbox" checked={selectedIds.has(group.id)} onChange={() => toggleSelected(group.id)} className="mr-2 h-6 w-6 align-top"/><OutputAssetCell asset={asset(group, 'AI')} isExporting={isCurrentExportGroup} /></td>{!isSimpleOutput ? <><td className="px-3 py-4"><OutputAssetCell asset={backAsset} isExporting={isCurrentExportGroup} /></td><td className="px-3 py-4"><OutputAssetCell asset={frontAsset} isExporting={isCurrentExportGroup} /></td><td className="px-3 py-4"><OutputAssetCell asset={lazerAsset} isExporting={isCurrentExportGroup} /></td><td className="px-4 py-4"><button type="button" disabled={operationLocked} onClick={() => setModalGroupId(group.id)} className="inline-flex h-12 w-[150px] items-center justify-center rounded-2xl border border-blue-300 text-sm font-semibold text-blue-700 disabled:opacity-40">{toolStatus.running && toolStatus.run?.kind === 'tool' ? 'Tool đang chạy' : toolStatus.running ? 'Đang export...' : exportingId === group.id ? 'Đang gửi...' : missingAny ? 'Export file' : 'Chọn export'}</button></td></> : null}</tr>;})}</DataTable><PaginationBar page={paged.currentPage} totalPages={paged.totalPages} totalItems={paged.total} pageSize={pageSize} onPageChange={setPage} onPageSizeChange={(size) => { setPageSize(size); setPage(1); }} /></Panel>{modalGroup ? <ExportModal fileName={modalGroup.fileName} onClose={() => setModalGroupId(null)} onSubmit={(assets) => runExport(modalGroup, assets)} /> : null}</div>;
}



function ApproveModal({ fileName, approving, onClose, onConfirm }: { fileName: string; approving: boolean; onClose: () => void; onConfirm: () => void }) {
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-label="Xác nhận bỏ qua lỗi"><div className="w-full max-w-md rounded-[28px] bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.25)]"><div className="text-xl font-semibold text-slate-900">Bạn chấp nhận bỏ qua lỗi của file này chứ?</div><p className="mt-3 break-all text-sm leading-6 text-slate-600">Ảnh <b>{fileName}</b> sẽ được chuyển về <code>Images</code> và đánh dấu bỏ qua check.</p><p className="mt-2 text-sm text-slate-500">Lần chạy tiếp theo Tool vẫn trace, sắp xếp và xuất file bình thường.</p><div className="mt-6 grid grid-cols-2 gap-3"><button type="button" disabled={approving} onClick={onClose} className="h-12 rounded-2xl border border-slate-300 text-sm font-semibold text-slate-700 disabled:opacity-40">No</button><button type="button" disabled={approving} onClick={onConfirm} className="h-12 rounded-2xl bg-amber-600 text-sm font-semibold text-white hover:bg-amber-700 disabled:opacity-40">{approving ? 'Đang approve...' : 'Yes'}</button></div></div></div>;
}
